<x-base-layout :scrollspy="false">
        ...
        <div class="middle-content container-xxl p-0">
            ...
            <x-widgets._w-breadcrumb>
                <li class="breadcrumb-item active"
                    aria-current="page">{{ __('general.menu.project_management.project') }}</li>
            </x-widgets._w-breadcrumb>
            <!-- /BREADCRUMB -->
                        ...
                        <div class="row">
                            @can(Acl::PERMISSION_BANNER_ADD)
                                <div class="col-xl-12 col-md-12 col-sm-12 col-12">
                                    <a href="{{ route('admin.project.create') }}" class="btn btn-secondary mt-4 ms-4 me-4">New</a>
                                </div>
                            @endcan
                        </div>
    ...
    <x-slot:footerFiles>
        ...
        <script>
            let drawDT = 0;
            const projectTable = $('#zero-config').DataTable({
                ...
                "columns": [
                    ...
                    },
                    {
                        ...
                        "render": function (data, type, full) {
                            ...
                            <p class="mb-0 pt-1 pb-1">
                                <span>{{ __('general.common.title') }}:</span>
                                <span style="white-space: normal">${full.title}</span>
                            </p>
                            <p class="mb-0 pt-1 pb-1">
                                <span>{{ __('general.common.description') }}:</span>
                                <span style="white-space: normal">${full.description}</span>
                            </p>`;
                        }
                    },
                    {
                        ...
                        "orderable":false,
                        "render": function(data,type,full) {
                            return `<span style="white-space: normal">${full.project_type_id}</span>`
                        }
                    },
                    {
                        "data": "platform",
                        ...
                        "render": function (data, type, full) {
                            return `<p class="mb-0 pt-1 pb-1 style="white-space: normal">
                                ${full.platform ? (full.platform == 1 ? 'Web' : (full.platform == 2 ? 'App' : 'Web, App')) : 'N/A'}
                                </p>`
                        }
                    },
                    {
                    ...
                    },
                    {
                        ...
                        "render": function (data, type, full) {
                            ...
                            let editUrl = "{{ route('admin.project.edit', ':id') }}";
                            editUrl = editUrl.replace(':id', full.id);

                            let destroyUrl = "{{ route('admin.project.destroy', ':id') }}";
                            ...
                            <div class="action-btns">
                                ...
                                <a href=${editUrl} class="action-btn btn-edit bs-tooltip me-2" data-url="${editUrl}" data-toggle="tooltip" data-placement="top" title="Edit">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24"
                                         viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"
                                         stroke-linecap="round" stroke-linejoin="round"
                                ...
                                @can(Acl::PERMISSION_PROJECT_DELETE)
        ...
        <script>
            $(document).on('change', '.toggle-status', function(e) {
                ...
                let url = '{{ route('admin.project.toggle_status', ':id') }}';
                url = url.replace(':id', project);
            ...
            $(document).on('click', '.btn-delete', function(e) {
                ...
                console.log(url)
                deleteItem(projectTable, url);
            });
        </script>
        <script>
            function linkBuilder(full) {
                var builder ="";
                if (full.platform==1||full.platform==3){
                builder+= `<div class="td-content"><span>Web:</span><a href="${full.link_web}" class="text-danger"><svg
                ...
                if (full.platform==2||full.platform==3){
                builder+=`<div class="td-content"><span>App:</span><a href="${full.link_app}" class="text-danger"><svg
                            xmlns="http://www.w3.org/2000/svg" width="24" height="24"
                            viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"
                            stroke-linecap="round" stroke-linejoin="round"
                            class="text-danger feather feather-chevrons-right">
